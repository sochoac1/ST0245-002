...
public class Octree
{
     ...
     * @see nuevoOct
     */
    public void octree (LinkedList<Bee> abejas,ArrayList<Double> mins,double midD,double midW,double midH) { //T(n) = 8*T(n/8) + c
        
        this.midD=midD; //C1
        this.midW=midW; //C2
        this.midH=midH; //C3
        ArrayList<LinkedList<Bee>> octree=new ArrayList(8); //Octree //C4
        //Inicializa valores del Octree
        for (int i=0;i<8;++i) { // C5*8
            LinkedList<Bee> ab=new LinkedList(); //C6*8
            octree.add(ab); //C7*8
        }
        for (int i=0;i<abejas.size();++i) { //C8*n
            Bee abeja=abejas.poll(); //C9*n
            int sector=hashing(abeja,mins); //C10*n O(1)
            octree.get(sector).addFirst(abeja); //C11*n O(1)
        }
        double ph=Math.sqrt(Math.pow((midD)*111325,2)+Math.pow((midW)*111325,2)); //Primera hipotenusa //C12*n
        double diagonal=Math.sqrt(Math.pow(ph,2)+Math.pow((midH),2)); //C13*n
        if (diagonal>100) { //C14
            for (int i=0;i<8;++i) {  //C15*8
                if (octree.get(i).size()>1) {
                    nuevoOctree(octree.get(i),mins,i); //T(n) = 8*T(n/8) + c (Esta es la complejidad para el peor de los casos)
                }
            }
        } else {
            for (int i=0;i<8;++i) {
