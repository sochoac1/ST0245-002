...
public class VehiculosCompartidos
{
    ...
    public static LinkedList<LinkedList<Integer>> asignarVehiculos(Graph grafo, float p){
        ...
        int contador = 1; //Personas por carro
        int tiempoActual = 0; //Tiempo total en recoger a cada persona
        float tiempoLimite = 0; //Tiempo en llegar a la Universidad por la constante p
        LinkedList<Integer> permutacion = new LinkedList<Integer>();        
        while (dueno < grafo.size()){
            if (contador == 1){ // Si el contador es 1, crear una nueva permutacion (No ha recogido a nadie)
                tiempoLimite = grafo.getWeight(dueno, 1) * p; //Se halla el tiempo en llegar a la Universidad por la P
                permutacion = new LinkedList<Integer>();
                permutacion.add(dueno); //Añade al conductor
                dueno++; 
                contador++;
                tiempoActual = 0; //No ha recogido a nadie entonces es cero
            } else { // Sino, seguir insertando en la permutacion actual //Se va a ver si puede recoger a alguien
                tiempoActual = tiempoActual + grafo.getWeight((dueno-1), dueno);
                if((tiempoActual + grafo.getWeight(dueno, 1)) < tiempoLimite){//Identificar si el tiempo transcurrido tras recoger a las personas no pasa el limite
                    permutacion.add(dueno);
                    dueno++;
                    contador++;
                    if(contador == 6 || dueno == grafo.size()){
                        contador = 1;
